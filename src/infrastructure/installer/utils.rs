...
pub fn extract_zip(zip_path: &Path, dest_dir: &Path) -> Result<(), String> {
    ...
    let mut archive = zip::ZipArchive::new(file).map_err(|e| format!("Failed to read ZIP file: {e}"))?;
...
pub fn extract_tar_gz(tar_path: &Path, dest_dir: &Path) -> Result<(), String> {
    let tar_path_str = safe_path_to_str(tar_path).map_err(|e| format!("Path conversion failed: {e}"))?;
    let dest_dir_str = safe_path_to_str(dest_dir).map_err(|e| format!("Dest path conversion failed: {e}"))?;
