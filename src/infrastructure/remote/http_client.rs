...
impl NetworkUtils {
    /// 检查网络连接
    pub async fn check_connectivity() -> Result<(), NetworkError> {
        let client = HttpClient::with_timeout(10).map_err(|e| NetworkError::GenericError(e.to_string()))?;
        ...
        for url in &urls {
            if client.check_url(url).await.unwrap_or(false) {
                return Ok(());
            }
        }
    ...
    /// 检查特定主机是否可达
    pub async fn check_host(url: &str) -> Result<(), NetworkError> {
        let client = HttpClient::new().map_err(|e| NetworkError::GenericError(e.to_string()))?;
    ...
    /// 测试连接速度
    pub async fn test_speed(url: &str) -> Result<Duration, NetworkError> {
        let start = std::time::Instant::now();
        let client = HttpClient::new().map_err(|e| NetworkError::GenericError(e.to_string()))?;

        match client.get(url).await {
            Ok(_) => Ok(start.elapsed()),
            Err(e) => Err(NetworkError::GenericError(e.to_string())),
        }
    }
}